...
void UnblockTasks(void)
{
	...
	{
		if(UserTasks[Local_u8TaskCounter].CurrentState == BlockState)
		{
			if(UserTasks[Local_u8TaskCounter].BLockCount == GlobalTickCounts)
			{
...
void SysTick_Handler(void)
{
	...
	/*2- Unblock Tasks*/
	UnblockTasks();
	/*3- Trigger the PendSV
	 * (note: PendSV is the lowest priority
	 * so it won't interrupt the systick handler
	 * and will come after all ISRs)*/
	SCB->SCB_ICSR |= (1 << 28);
...
{
	if(CurrentTask != 0u)    /* Idle task can't be blocked state*/
	{
